...
Si une information n'est pas visible ou lisible, mets null pour cette valeur."""
        ...
        try:
            ...
            if content and len(content) > 0:
                ...
                # Date de naissance (nouveau champ)
                if data.get('date_naissance'):
                    try:
                        # Format attendu: JJ.MM.AAAA ou JJ.MM.AA
                        date_str = data['date_naissance']
                        ...
                        if len(parts) == 3:
                            ...
                            update_vals['id_card_birthdate'] = date(int(year), int(month), int(day))
                    except:
                        pass
                
                # Lieu de naissance (nouveau champ)
                if data.get('lieu_naissance'):
                    update_vals['id_card_birthplace'] = data['lieu_naissance']
                
                # Sexe (nouveau champ)
                if data.get('sexe'):
                    sexe = data['sexe'].upper().strip()
                    if sexe in ['M', 'H', 'HOMME', 'MASCULIN']:
                        update_vals['id_card_gender'] = 'M'
                    elif sexe in ['F', 'V', 'FEMME', 'FEMININ']:
                        update_vals['id_card_gender'] = 'F'
                
                # Nationalité (nouveau champ)
                if data.get('nationalite'):
                    update_vals['id_card_nationality'] = data['nationalite']
                
                # Date de validité (nouveau champ)
                if data.get('date_validite'):
                    try:
                        date_str = data['date_validite']
                        parts = date_str.replace('/', '.').replace('-', '.').split('.')
                        if len(parts) == 3:
                            day, month, year = parts
                            if len(year) == 2:
                                year = '20' + year
                            from datetime import date
                            update_vals['id_card_expiry_date'] = date(int(year), int(month), int(day))
                    except:
                        pass
