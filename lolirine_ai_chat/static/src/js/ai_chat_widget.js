...
(function() {
    ...
    var STORAGE_KEY_TEASER = "lai_teaser_last_shown";
    var STORAGE_KEY_TEASER_IDX = "lai_teaser_index";

    var TEASER_MESSAGES = [
        "\ud83d\udc4b Bonjour\u00a0! Besoin d\u2019aide pour votre piscine\u00a0? Je suis l\u00e0 pour vous aider\u00a0!",
        "\ud83d\udc20 Traitement de l\u2019eau, filtration, robots nettoyeurs\u2026 Posez-moi vos questions\u00a0!",
        "\u2600\ufe0f Pr\u00e9parez votre saison piscine\u00a0! Je peux vous conseiller sur l\u2019entretien et les accessoires.",
        "\ud83d\udca7 Une question sur le pH, le chlore ou l\u2019hivernage\u00a0? Je suis votre assistant piscine\u00a0!",
        "\u2753 Besoin d\u2019un conseil\u00a0? Cliquez pour discuter avec notre assistant piscine\u00a0!",
    ];
    ...
    var QUICK_ACTIONS = [
        ...
        { label: "\ud83d\udd25 Chauffage", prompt: "Quelles solutions de chauffage piscine?" },
        { label: "\ud83d\udec1 Spas", prompt: "Quels spas et jacuzzis avez-vous?" },
        { label: "\u2744\ufe0f Hivernage", prompt: "Comment hiverner ma piscine?" },
        { label: "\ud83c\udfca Hors-sol", prompt: "Quelles piscines hors-sol disponibles?" },
    ];
    ...
    Chat.prototype._getSid = function() {
        var s = sessionStorage.getItem("lai_sid");
    ...
    Chat.prototype.init = function() {
        ...
        .then(function(c) {
            ...
            // Auto teaser after delay
            self._scheduleTeaser();
        })
        .catch(function(e) { console.warn("[AI Chat] Init skipped:", e.message); });
    };

    Chat.prototype._build = function() {
        this.el = document.createElement("div");
        this.el.id = "lolirine-ai-chat-container";
        this.el.innerHTML =
            '<div class="lai-chat-window" style="display:none">' +
            '<div class="lai-header">' +
            '<div class="lai-header-info">' +
            '<div class="lai-header-avatar">\ud83c\udfca</div>' +
            '<div><div class="lai-header-title">Lolirine Pool Store</div>' +
            '<div class="lai-header-status"><span class="lai-status-dot"></span>Assistant IA</div>' +
            '</div></div>' +
            ...
            '<div class="lai-footer">Lolirine Pool Store \u00a9 2026</div>' +
            '</div></div>' +
    ...
    Chat.prototype._clear = function() {
        ...
        jsonRpc("/ai_chat/close", {session_id: this.sid}).catch(function(){});
        this.sid = "lai_" + Date.now() + "_" + Math.random().toString(36).substr(2,9);
        sessionStorage.setItem("lai_sid", this.sid);
        this.area.innerHTML = "";
        ...
        for (var i = 1; i <= 5; i++) {
            (function(v) {
                ...
                s.addEventListener("click", function() {
                    self.rated = true;
                    jsonRpc("/ai_chat/rate", {session_id: self.sid, rating: v}).catch(function(){});
    ...
    Chat.prototype._shouldShowTeaser = function() {
        var interval = parseInt(this.cfg.teaser_interval || 24, 10);
        if (interval === 0) return false;
        var last = localStorage.getItem(STORAGE_KEY_TEASER);
        if (!last) return true;
        var elapsed = (Date.now() - parseInt(last, 10)) / 3600000;
        return elapsed >= interval;
    };

    Chat.prototype._nextTeaserMessage = function() {
        var idx = parseInt(localStorage.getItem(STORAGE_KEY_TEASER_IDX) || "0", 10);
        var msg = TEASER_MESSAGES[idx % TEASER_MESSAGES.length];
        localStorage.setItem(STORAGE_KEY_TEASER_IDX, String((idx + 1) % TEASER_MESSAGES.length));
        return msg;
    };

    Chat.prototype._scheduleTeaser = function() {
        var self = this;
        if (!this._shouldShowTeaser()) return;
        if (self.isOpen) return;
        var delay = parseInt(self.cfg.teaser_delay || 5, 10) * 1000;
        setTimeout(function() {
            if (self.isOpen) return;
            self._showTeaser();
        }, delay);
    ...
    Chat.prototype._showTeaser = function() {
        ...
        localStorage.setItem(STORAGE_KEY_TEASER, String(Date.now()));

        // Start pulse on toggle button
        ...
        // Click teaser = open chat
        teaser.addEventListener("click", function(e) {
            if (e.target.classList.contains("lai-teaser-close")) {
                self._hideTeaser();
                return;
            }
            self._hideTeaser();
        ...
        this.teaserTimer = setTimeout(function() {
            self._hideTeaser();
        }, 12000);
    };
