...
class PoolCatalogPdfImport(models.Model):
    ...
    def action_extract_images_lite(self):
        ...
        # Le diagnostic a révélé que le PDF est dans le champ 'source_pdf'
        if not self.source_pdf:
            raise UserError("Aucun fichier PDF n'est attaché à cet import (champ source_pdf vide).")
        
        _logger.info(f"PDF trouvé dans le champ: source_pdf ({self.source_pdf_filename})")
        
        if hasattr(self, 'image_extraction_state') and self.image_extraction_state == 'in_progress':
            raise UserError("Une extraction est déjà en cours. Attendez qu'elle se termine.")
        
        try:
            if hasattr(self, 'image_extraction_state'):
                self.write({
                    'image_extraction_state': 'in_progress',
                    'image_extraction_progress': 0.0
                })
            
            # Essayer différents formats pour ouvrir le PDF
            doc = None
            pdf_data = None
            
            # Format 1: Données binaires directes
            _logger.info(f"Tentative 1: Données binaires directes (taille: {len(self.source_pdf)} bytes)")
            try:
                doc = fitz.open(stream=self.source_pdf, filetype="pdf")
                pdf_data = self.source_pdf
                _logger.info("✅ PDF ouvert avec données binaires directes")
            except Exception as e:
                _logger.info(f"❌ Échec données binaires directes: {e}")
            
            # Format 2: Base64 décodé
            if not doc:
                _logger.info("Tentative 2: Décodage Base64")
                try:
                    import base64
                    decoded_data = base64.b64decode(self.source_pdf)
                    doc = fitz.open(stream=decoded_data, filetype="pdf")
                    pdf_data = decoded_data
                    _logger.info("✅ PDF ouvert avec décodage Base64")
                except Exception as e:
                    _logger.info(f"❌ Échec décodage Base64: {e}")
            
            # Format 3: UTF-8 puis Base64
            if not doc and isinstance(self.source_pdf, bytes):
                _logger.info("Tentative 3: UTF-8 vers string puis Base64")
                try:
                    import base64
                    str_data = self.source_pdf.decode('utf-8')
                    decoded_data = base64.b64decode(str_data)
                    doc = fitz.open(stream=decoded_data, filetype="pdf")
                    pdf_data = decoded_data
                    _logger.info("✅ PDF ouvert avec UTF-8->Base64")
                except Exception as e:
                    _logger.info(f"❌ Échec UTF-8->Base64: {e}")
            
            # Format 4: Diagnostic approfondi
            if not doc:
                _logger.info("Diagnostic approfondi des données:")
                _logger.info(f"  Type: {type(self.source_pdf)}")
                _logger.info(f"  Taille: {len(self.source_pdf)}")
                if hasattr(self.source_pdf, '__class__'):
                    _logger.info(f"  Classe: {self.source_pdf.__class__}")
                
                # Vérifier les premiers bytes
                if isinstance(self.source_pdf, bytes):
                    first_bytes = self.source_pdf[:20]
                    _logger.info(f"  Premiers bytes: {first_bytes}")
                    if first_bytes.startswith(b'%PDF'):
                        _logger.info("  -> Commence par %PDF (bon format)")
                    else:
                        _logger.info("  -> Ne commence pas par %PDF")
                elif isinstance(self.source_pdf, str):
                    first_chars = self.source_pdf[:20]
                    _logger.info(f"  Premiers chars: {repr(first_chars)}")
                
                raise UserError("Impossible d'ouvrir le PDF avec tous les formats testés. Consultez les logs pour le diagnostic.")
            
            if not doc:
                raise UserError("Erreur lors de l'extraction d'images: Failed to open stream")
            total_pages = len(doc)
            
            _logger.info(f"Début extraction images PDF {self.source_pdf_filename}: {total_pages} pages")
    ...
    def _action_view_extracted_images(self):
        ...
        return {
            ...
            'name': f'Images extraites - {self.source_pdf_filename}',
