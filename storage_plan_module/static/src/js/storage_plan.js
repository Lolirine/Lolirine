...
(function() {
    ...
    var MODAL_BLOCK_DURATION = 5000; // 5 secondes de blocage après fermeture

    // Attendre que le DOM soit chargé
    $(document).ready(function() {
        console.log('Storage Plan JS loaded v1.0.18');
    ...
    function initializeStoragePlan() {
        ...
        // ============================================
        // DÉTECTION DU MODE ÉDITION - TRÈS EXHAUSTIVE
        // ============================================
        function isEditMode() {
            // Vérifier toutes les conditions possibles du mode édition Odoo
            var editModeDetected = 
                // Classes sur body
                $('body').hasClass('editor_enable') ||
                $('body').hasClass('o_we_command_open') ||
                $('body').hasClass('editor_has_snippets') ||
                $('body').hasClass('o_website_designer_backend') ||
                
                // URL avec paramètre d'édition
                window.location.search.indexOf('enable_editor') > -1 ||
                window.location.hash.indexOf('enable_editor') > -1 ||
                
                // Éléments de l'éditeur présents
                $('#oe_manipulators').length > 0 ||
                $('.o_we_website_top_actions').length > 0 ||
                $('.o_website_preview').length > 0 ||
                $('#web_editor-top-edit').length > 0 ||
                $('.o_we_ui_loading').length > 0 ||
                $('.o_we_customize_panel').length > 0 ||
                $('.oe_overlay').length > 0 ||
                $('[data-oe-model]').length > 0 ||
                
                // Panneau d'édition visible
                $('.o_we_customize_panel:visible').length > 0 ||
                $('#o_we_editor_toolbar:visible').length > 0 ||
                
                // Iframe d'édition
                $('iframe.o_website_preview').length > 0;
        ...
        // ============================================
        // Fermer la modal dès qu'on entre en mode édition
        setInterval(function() {
            if (isEditMode()) {
                ...
                // Toujours bloquer en mode édition
                if (!modalBlocked) {
                    blockModal(10000);
                }
            }
        ...
        // Observer les changements sur body pour détecter le mode édition
        if (typeof MutationObserver !== 'undefined') {
            var observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                    if (mutation.attributeName === 'class') {
                        if (isEditMode()) {
                            console.log('Body class changed to edit mode');
                            closeModal();
                            blockModal(10000); // Bloquer 10 secondes
                        }
                    }
                });
            });
        ...
        // ============================================
        // Quand on clique sur n'importe quel bouton de l'éditeur
        $(document).on('click', '.o_we_website_top_actions button, .o_we_website_top_actions a', function() {
            console.log('Editor button clicked, blocking modal');
            closeModal();
            blockModal(10000); // Bloquer 10 secondes
        });
