...
class LaunchSearchWizard(models.TransientModel):
    ...
    only_catalog_imported = fields.Boolean(
        string='Uniquement produits importés du catalogue', default=True,
        help="Filtrer sur x_pool_import_date != False (produits avec ref supplier connue)"
    )
    ...
    enable_bg_removal = fields.Boolean(string='Background removal', default=False,
        help="Lourd : ralentit le scraping. Active uniquement si rembg est installé.")
    ...
    def action_launch(self):
        ...
        if self.only_catalog_imported:
            # Garde seulement les produits avec une ref supplier (issus du catalogue)
            products = products.filtered(
                lambda p: bool(getattr(p, 'x_pool_supplier_ref', False))
            )
