...
class ProductTemplate(models.Model):
    ...
    def action_manage_variant_images(self):
        """Ouvre le wizard pour gérer les images de chaque variante."""
        self.ensure_one()
        wizard = self.env['variant.image.wizard'].create({
            'product_tmpl_id': self.id,
        })
        # Créer une ligne pour chaque variante
        for variant in self.product_variant_ids.sorted(lambda v: v.display_name):
            # Construire le libellé des attributs
            attr_parts = []
            for ptav in variant.product_template_attribute_value_ids:
                attr_parts.append('%s: %s' % (
                    ptav.attribute_id.name,
                    ptav.product_attribute_value_id.name,
                ))
            attr_str = ' | '.join(attr_parts) if attr_parts else variant.display_name

            self.env['variant.image.wizard.line'].create({
                'wizard_id': wizard.id,
                'variant_id': variant.id,
                'variant_name': variant.display_name,
                'attribute_values': attr_str,
            })
        return {
            'name': 'Images des variantes - %s' % self.name,
            'type': 'ir.actions.act_window',
            'res_model': 'variant.image.wizard',
            'res_id': wizard.id,
            'view_mode': 'form',
            'target': 'new',
        }
