...
class SaleOrderPool(models.Model):
    ...
    def create(self, vals_list):
        """
        Force la séquence PSC AVANT super() pour prendre la main
        sur toute autre séquence définie en aval (ex: CTR de lolirine_contract).
        """
    ...
    def write(self, vals):
        """Si is_pool_quote activé sur un devis à '/', assigner PSC."""
        if vals.get('is_pool_quote'):
            for rec in self:
