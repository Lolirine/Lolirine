...
class LolirineScanTva(models.Model):
    ...
    def _save_to_documents(self):
        ...
        # Vérifier si le module Documents est installé
        ...
        if not module_installed:
            _logger.info("Module Documents non installe")
            return False
        
        # Vérifier si les modèles existent
        model_exists = self.env['ir.model'].sudo().search([
            ('model', '=', 'documents.folder')
        ], limit=1)
        
        if not model_exists:
            _logger.info("Modele documents.folder non trouve dans ir.model")
            return False
        ...
        try:
            ...
            if not folder:
                ...
                _logger.info("Dossier Finance trouve: %s", parent_folder)
                
                # Si pas trouvé, chercher d'autres dossiers
                ...
                if not parent_folder:
                    ...
                    _logger.info("Premier dossier disponible: %s", parent_folder)
                
                if not parent_folder:
                    _logger.error("Aucun dossier Documents trouve - impossible de creer 'Scans TVA'")
                    return False
                ...
                _logger.info("Dossier 'Scans TVA' cree dans '%s' (ID: %s)", parent_folder.name, folder.id)
        ...
        try:
            ...
            if result and self.documents_document_id:
                ...
                return {
                    ...
                    'params': {
                        'title': _("Erreur"),
                        'message': _("Impossible d'archiver le document. Consultez les logs Odoo pour plus de details."),
                        'type': 'warning',
                        'sticky': True,
                    }
                }
