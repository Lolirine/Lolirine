...
class PoolImageSearchSession(models.Model):
    ...
    enable_bg_removal = fields.Boolean(string='Background removal (rembg)', default=False,
                                       help="Désactivé par défaut : ralentit fortement le scraping et "
                                            "rembg peut ne pas être installé. Activable session par session.")
    ...
    def action_queue(self):
        ...
        for session in self:
            if not session.product_ids:
                raise ValidationError("Aucun produit sélectionné.")
            session.write({
                'state': 'queued',
    ...
    def _process_product(self, product, orchestrator, processor):
        ...
        if not raw_candidates:
            _logger.info(
                "[%s] %s -> 0 candidates (%.1fs)",
                product.default_code or product.id,
                product.name[:40] if product.name else '',
                time.time() - t_start,
            )
            return
        ...
        _logger.info(
            "[%s] %s -> %d candidates kept (best score=%.0f) auto=%s (%.1fs)",
            product.default_code or product.id,
            product.name[:40] if product.name else '',
            len(created),
            created[0].score if created else 0,
            'YES' if (created and created[0].auto_validated) else 'no',
            time.time() - t_start,
        )
