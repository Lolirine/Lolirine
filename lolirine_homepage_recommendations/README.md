# Lolirine Pool - Homepage Recommendations

Module Odoo 19 de recommandations personnalisées inspiré d'Amazon **exclusivement pour le site Lolirine Pool**.

## ⚠️ IMPORTANT - Site spécifique

Ce module est conçu **uniquement pour le site Lolirine Pool** et ne fonctionnera pas sur le site de stockage (garde-meubles). Toutes les données de tracking et les recommandations sont filtrées par website.
...
- ✅ Tracking automatique des vues de produits **sur Lolirine Pool uniquement**
...
- ✅ **Isolation complète des données par website**
...
3. Installer le module via l'interface Odoo :
   - Allez dans Apps
   - Cherchez "Lolirine Pool - Homepage Recommendations"
   - Cliquez sur Installer

### ⚙️ Configuration obligatoire post-installation

**ÉTAPE CRITIQUE : Configurer l'ID du website Lolirine Pool**

1. Trouvez l'ID du website Lolirine Pool :
   - Allez dans **Website > Configuration > Websites**
   - Ouvrez le site **Lolirine Pool**
   - Notez l'ID dans l'URL : `/web#id=X&model=website` (X est l'ID)

2. Configurez le paramètre système :
   - Allez dans **Paramètres > Technique > Paramètres système**
   - Cherchez `lolirine_pool.website_id`
   - Remplacez la valeur `0` par l'ID du website (ex: `2`)

3. (Optionnel) Vérifiez le domaine :
   - Cherchez `lolirine_pool.domain`
   - Vérifiez que la valeur correspond à votre domaine (ex: `pool.lolirine.be`)

**Sans cette configuration, le module utilisera le domaine pour identifier le site, ce qui peut être moins fiable.**
