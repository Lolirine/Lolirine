...
class PoolCatalogExtractionProduct(models.Model):
    ...
    def action_import_to_odoo(self):
        ...
        # Préparer les valeurs de base (champs standard Odoo)
        vals = {
            'name': self.name or 'Produit sans nom',
            'default_code': f"POOL-{self.reference or self.type_code or str(self.id)}",
        ...
        if 'detailed_type' in ProductTemplate._fields:
            vals['detailed_type'] = 'product'  # Odoo 15+
        elif 'type' in ProductTemplate._fields:
            vals['type'] = 'product'  # Odoo 14 et avant
        
        # Ajouter les champs personnalisés s'ils existent sur le modèle
        custom_fields = {
            'x_pool_supplier_ref': self.reference or self.type_code or '',
            ...
            'x_power_kw': float(self.power_kw or 0),
            'x_power_cv': self.power_cv or '',
            'x_voltage': int(self.voltage or 0),
            'x_flow_rate': float(self.flow_rate or 0),
            'x_diameter_mm': int(self.diameter_mm or 0),
            'x_filter_area': float(self.filter_area or 0),
            'x_cop': float(self.cop or 0),
            'x_noise_level': float(self.noise_level or 0),
        }
        
        # Vérifier quels champs personnalisés existent
        for field_name, field_value in custom_fields.items():
            if field_name in ProductTemplate._fields:
                vals[field_name] = field_value
        
        # Ajouter le fournisseur si le champ existe et qu'on a un fournisseur
        ...
        try:
            ...
            # Retourner True pour les appels JavaScript (pas d'action window)
            return True
            
        except Exception as e:
            _logger.error(f"Erreur import produit: {str(e)}")
            self.write({
                'state': 'error',
                'error_message': str(e),
            })
