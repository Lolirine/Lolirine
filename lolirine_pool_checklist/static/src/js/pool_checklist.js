...
function ProductPanel({ item, sectionLabel, onAdd, onClose }) {
  ...
  async function run(query) {
    ...
    try {
      const r=await fetch(cfg.productsEndpoint||'/pool-checklist/products',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',method:'call',id:1,params:{query,limit:15}})});
      const d=await r.json();
      const prods=d?.result?.products||[];
      if(prods.length){setRes(prods);setSrc('odoo');setBusy(false);return;}
    } catch {}
    try {
      const r=await fetch(cfg.aiEndpoint||'/pool-checklist/ai-suggest',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',method:'call',id:1,params:{item_text:query,section_label:sectionLabel}})});
      const d=await r.json();
      const prods=d?.result?.products||[];
  ...
  function addSel(){const chosen=results.filter((_,i)=>sel[i]);if(chosen.length)onAdd(chosen);}
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,.5)',zIndex:9990,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'#fff',borderRadius:15,width:'min(660px,96vw)',maxHeight:'88vh',display:'flex',flexDirection:'column',boxShadow:'0 24px 80px rgba(0,0,0,.22)'}}>
        <div style={{padding:'15px 18px 10px',borderBottom:'1px solid #f0f4f8',display:'flex',gap:10,alignItems:'flex-start'}}>
          <div style={{flex:1}}><div style={{fontWeight:700,fontSize:15,color:'#1e293b'}}>🔍 Produits associés</div><div style={{fontSize:11,color:'#64748b',marginTop:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{item}</div></div>
        ...
        <div style={{padding:'10px 18px',borderBottom:'1px solid #f0f4f8',display:'flex',gap:8}}>
          <input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&run(q)} placeholder="Référence, nom produit, marque…" style={{flex:1,border:'1.5px solid #dde4ed',borderRadius:8,padding:'7px 12px',fontFamily:'inherit',fontSize:13,outline:'none'}} />
          <button onClick={()=>run(q)} style={{background:'#0ea5e9',color:'#fff',border:'none',borderRadius:8,padding:'7px 16px',fontWeight:600,cursor:'pointer',fontSize:13,whiteSpace:'nowrap'}}>{busy?'…':'Chercher'}</button>
        </div>
        {src&&src!=='empty'&&(<div style={{padding:'4px 18px',background:src==='odoo'?'#f0fdf4':'#fffbeb',borderBottom:'1px solid #f0f4f8'}}><span style={{fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:20,background:src==='odoo'?'#dcfce7':'#fef3c7',color:src==='odoo'?'#166534':'#92400e'}}>{src==='odoo'?'✅ Catalogue Lolirine Pool Store':'✨ Suggestions IA'}</span></div>)}
        <div style={{flex:1,overflowY:'auto',padding:'4px 18px'}}>
          ...
          {results.map((p,i)=>{const price=typeof p.price==='number'?p.price:(parseFloat(p.price)||0);const sup=p.suppliers?.[0]||{};return(
            <div key={i} onClick={()=>toggle(i)} style={{display:'flex',gap:10,padding:'8px 10px',margin:'3px 0',borderRadius:9,border:`1.5px solid ${sel[i]?'#0ea5e9':'#e8edf3'}`,background:sel[i]?'rgba(14,165,233,.05)':'#fff',cursor:'pointer',alignItems:'flex-start'}}>
              <div style={{width:17,height:17,border:`2px solid ${sel[i]?'#0ea5e9':'#bbb'}`,borderRadius:4,background:sel[i]?'#0ea5e9':'transparent',flexShrink:0,marginTop:2,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:11}}>{sel[i]&&'✓'}</div>
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:'#64748b',display:'flex',gap:9,flexWrap:'wrap',marginTop:2}}>
                  {p.ref&&<span>Réf: {p.ref}</span>}{p.category&&<span>· {p.category}</span>}{p.unit&&<span>· {p.unit}</span>}{sup.name&&<span style={{color:'#7c3aed'}}>· {sup.name}</span>}{price>0&&<span style={{color:'#16a34a',fontWeight:600}}>· {price.toFixed(2)} €</span>}
                </div>
              </div>
            </div>
          );})}
        </div>
        {results.length>0&&(<div style={{padding:'10px 18px',borderTop:'1px solid #f0f4f8',display:'flex',justifyContent:'flex-end',gap:9}}>
          ...
          <button onClick={addSel} disabled={!nSel} style={{background:nSel?'#0ea5e9':'#cbd5e1',color:'#fff',border:'none',borderRadius:8,padding:'7px 18px',fontWeight:700,cursor:nSel?'pointer':'default',fontSize:13}}>Ajouter {nSel?`(${nSel})`:''}</button>
        </div>)}
      </div>
    </div>
  );
