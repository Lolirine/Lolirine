...
class PoolImportWizard(models.TransientModel):
    ...
    # Alias pour compatibilité avec anciennes vues en cache
    ...
    # Alias pour compatibilité avec anciennes vues en cache
    file_data = fields.Binary(related='import_file', string='Données fichier', readonly=False)
    data_file = fields.Binary(related='import_file', string='Fichier données', readonly=False)
    file = fields.Binary(related='import_file', string='File', readonly=False)
    filename = fields.Char(related='import_filename', string='Filename', readonly=False)
    ...
    # Options Dropshipping
    create_dropship_info = fields.Boolean(
        string='Créer info dropshipping',
        default=True,
        help="Crée automatiquement les informations fournisseur dropshipping pour chaque produit importé"
    )
    default_dropship_delay = fields.Integer(
        string='Délai livraison par défaut (jours)',
        default=5,
        help="Délai de livraison estimé pour les produits dropshipping"
    )
    default_shipping_cost = fields.Float(
        string='Frais de port par défaut',
        default=0.0,
        help="Frais de port estimés pour le dropshipping"
    )
    ...
    def _import_json(self, content, log):
        ...
        for product_data in data.get('products', []):
            ...
            try:
                ...
                # Créer/mettre à jour les infos dropshipping
                if action in ('create', 'update'):
                    self._create_dropship_info(product, product_data)
        ...
        if self.import_templates:
            for template_data in data.get('productTemplates', []):
                try:
                    ...
                    # Créer les infos dropshipping pour le template
                    base_variant = template_data.get('variants', [{}])[0]
                    self._create_dropship_info(
                        template, 
                        purchase_price=base_variant.get('purchasePrice', 0),
        ...
        for row in reader:
            ...
            try:
                ...
                if action in ('create', 'update'):
                    self._create_dropship_info(
                        product, 
                        purchase_price=vals.get('standard_price', 0),
                        supplier_ref=ref
                    )
    ...
    def _create_dropship_info(self, product, product_data=None, purchase_price=None, supplier_ref=None):
        """Crée ou met à jour les informations dropshipping pour un produit"""
        if not self.create_dropship_info:
            return False
        
        # Récupérer le partenaire lié au fournisseur pool
        partner = self.supplier_id.partner_id
        if not partner:
            ...
            partner = self.env['res.partner'].create({
                ...
                'dropship_certified': True,
            })
            self.supplier_id.partner_id = partner
        else:
        ...
        price = purchase_price or (product_data.get('purchasePrice', 0) if product_data else product.standard_price)
        ref = supplier_ref or (product_data.get('ref') if product_data else product.x_pool_supplier_ref)
        ...
        dropship_vals = {
            ...
            'delay': self.default_dropship_delay,
            'shipping_cost': self.default_shipping_cost,
            'is_dropship_capable': True,
            'is_active': True,
            'min_order_qty': 1,
            'notes': f"Import automatique depuis {self.supplier_id.name} le {fields.Datetime.now().strftime('%d/%m/%Y')}",
        }
