...
class LolirineScanTva(models.Model):
    ...
    def _compute_documents_document_name(self):
        ...
        for record in self:
            if record.documents_document_id and 'documents.document' in self.env:
                try:
    ...
    def _save_to_documents(self):
        ...
        if 'documents.document' not in self.env:
            _logger.info("Module Documents non installe, sauvegarde ignoree")
            return False
    ...
    def action_validate(self):
        ...
        if self.documents_document_id and self.partner_id and 'documents.document' in self.env:
            try:
    ...
    def action_archive_document(self):
        ...
        # Vérifier que le module Documents est installé
        if 'documents.document' not in self.env:
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': _("Module Documents non installe"),
                    'message': _("Le module Documents n'est pas installe sur cette instance."),
                    'type': 'warning',
                    'sticky': False,
                }
            }
        ...
        try:
            ...
            if result and self.documents_document_id:
                ...
                return {
                    ...
                    'params': {
                        'title': _("Erreur"),
                        'message': _("Impossible d'archiver le document. Consultez les logs pour plus de details."),
                        'type': 'warning',
                        'sticky': True,
                    }
                }
