/**
 * Lolirine Storage Chat Widget - v2.0.0
 * Teaser avec localStorage + intervalle configurable
 */
(function() {
    "use strict";

    var STORAGE_KEY_TEASER = "lsc_teaser_last_shown";
    var STORAGE_KEY_TEASER_IDX = "lsc_teaser_index";

    var TEASER_MESSAGES = [
        "\ud83d\udc4b Bonjour\u00a0! Besoin d\u2019un espace de stockage\u00a0? Je peux vous renseigner sur nos box et tarifs.",
        "\ud83d\udce6 Saviez-vous\u00a0? Nos box sont accessibles 24h/24, 7j/7 sans frais suppl\u00e9mentaires\u00a0!",
        "\ud83d\udd12 S\u00e9curit\u00e9 maximale\u00a0: vid\u00e9osurveillance 24h, badge individuel, locaux chauff\u00e9s. Posez-moi vos questions\u00a0!",
        "\ud83d\udcb0 Des tarifs \u00e0 partir de 80\u20ac/mois. Je peux vous aider \u00e0 trouver le box id\u00e9al\u00a0!",
        "\u2753 Une question sur le stockage\u00a0? Je suis l\u00e0 pour vous aider\u00a0!",
    ];
    ...
    var QUICK_ACTIONS = [
        ...
        { label: "\ud83d\udd10 S\u00e9curit\u00e9", prompt: "Comment est assur\u00e9e la s\u00e9curit\u00e9 du garde-meuble?" },
        { label: "\ud83d\udcc5 Conditions", prompt: "Quelles sont les conditions de location?" },
        { label: "\ud83d\ude9a D\u00e9m\u00e9nagement", prompt: "Proposez-vous un service de d\u00e9m\u00e9nagement?" },
    ...
    /* ---- Teaser : logique localStorage + intervalle ---- */
    Chat.prototype._shouldShowTeaser = function() {
        var interval = parseInt(this.cfg.teaser_interval || 24, 10);
        if (interval === 0) return false; // désactivé

        var last = localStorage.getItem(STORAGE_KEY_TEASER);
        if (!last) return true; // jamais montré

        var elapsed = (Date.now() - parseInt(last, 10)) / 3600000; // en heures
    ...
    Chat.prototype._send = function() {
        ...
        .catch(function(e) {
            self._loader(false);
            self._bubble("assistant", "Erreur. R\u00e9essayez.", true);
        })
