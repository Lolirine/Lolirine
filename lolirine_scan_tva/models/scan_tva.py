...
class LolirineScanTva(models.Model):
    ...
    def _compute_documents_document_name(self):
        ...
        for record in self:
            if record.documents_document_id:
                try:
                    doc = self.env['documents.document'].browse(record.documents_document_id)
                    if doc.exists():
    ...
    def _save_to_documents(self):
        ...
        try:
            # Tester l'accès aux modèles Documents
            Document = self.env['documents.document']
            Folder = self.env['documents.folder']
            # Vérifier que les modèles sont vraiment accessibles
            if not Document._name or not Folder._name:
                raise KeyError("Models not accessible")
        except KeyError:
            _logger.info("Module Documents non installe ou non accessible, sauvegarde ignoree")
            return False
    ...
    def action_validate(self):
        ...
        if self.documents_document_id and self.partner_id:
            try:
                doc = self.env['documents.document'].browse(self.documents_document_id)
                if doc.exists():
                    doc.write({
