...
class ApartmentLease(models.Model):
    ...
    next_indexation_date = fields.Date(
        ...
        compute='_compute_next_indexation',
        store=True,
    )
    ...
    @api.depends('last_indexation_date', 'date_start', 'indexation_enabled')
    def _compute_next_indexation(self):
        for record in self:
            if not record.indexation_enabled or not record.date_start:
                record.next_indexation_date = False
                continue
            
            if record.last_indexation_date:
                record.next_indexation_date = record.last_indexation_date + relativedelta(years=1)
            else:
                # Première indexation: 1 an après le début du bail
                record.next_indexation_date = record.date_start + relativedelta(years=1)
