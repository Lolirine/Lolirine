...
class AiChatConversation(models.Model):
    ...
    message_ids = fields.One2many(
        'ai.chat.message', 'conversation_id', string='Messages',
    )
    ...
    @api.depends('message_ids', 'message_ids.role',
                 'message_ids.has_web_search', 'message_ids.create_date')
    def _compute_stats(self):
        for rec in self:
            msgs = rec.message_ids
            rec.message_count = len(msgs)
            rec.web_searches_count = len(msgs.filtered('has_web_search'))
