...
class AccountMove(models.Model):
    ...
    def _send_invoice_auto(self):
        ...
        try:
            ...
            # Chercher le template Lolirine et generer le contenu
            template = self.env.ref('lolirine_invoice.email_template_invoice', raise_if_not_found=False)
            if not template:
                template = self.env['mail.template'].search([
                    ('model_id.model', '=', 'account.move'),
                    ('name', '=', 'Facture mensuelle Lolirine')
                ], limit=1)
            
            if template:
                # Utiliser le template pour generer le sujet et le corps
                email_values = template.generate_email([self.id], ['subject', 'body_html', 'email_from'])
                email_data = email_values.get(self.id, {})
                subject = email_data.get('subject', f"Envoi de votre facture mensuelle {self.name} - Garde-meubles Lolirine")
                body_html = email_data.get('body_html', '')
                email_from = email_data.get('email_from', 'gardemeublelolirine@gmail.com')
            else:
