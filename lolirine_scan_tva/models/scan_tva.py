...
class LolirineScanTva(models.Model):
    ...
    # Lien vers le document dans l'app Documents (sauvegarde)
    # Note: Integer au lieu de Many2one pour éviter erreur si module Documents non installé
    documents_document_id = fields.Integer(
        string="ID Document archive",
        readonly=True,
        help="ID du document dans l'application Documents pour archivage"
    )
    documents_document_name = fields.Char(
        string="Document archive",
        compute="_compute_documents_document_name",
        store=False
    )
    ...
    @api.depends('documents_document_id')
    def _compute_documents_document_name(self):
        """Récupérer le nom du document archivé"""
        for record in self:
            if record.documents_document_id:
                # Vérifier si le modèle existe dans le registre
                try:
                    if self.env.registry.get('documents.document'):
                        doc = self.env['documents.document'].sudo().browse(record.documents_document_id)
                        if doc.exists():
                            record.documents_document_name = doc.name
                        else:
                            record.documents_document_name = False
                    else:
                        record.documents_document_name = False
                except Exception:
                    record.documents_document_name = False
    ...
    def action_scan_ocr(self):
        ...
        try:
            ...
            # Sauvegarder dans l'app Documents
    ...
    def _save_to_documents(self):
        ...
        # Vérifier si les modèles Documents existent dans le registre
        try:
            if not self.env.registry.get('documents.folder') or not self.env.registry.get('documents.document'):
                _logger.info("Module Documents non disponible dans le registre, sauvegarde ignoree")
                return False
        except Exception:
            _logger.info("Impossible de verifier le registre Documents")
            return False
        
        # Ne pas créer de doublon
        if self.documents_document_id:
            _logger.info("Document deja archive (ID: %s)", self.documents_document_id)
            return True
        ...
        try:
            # Accéder aux modèles Documents avec sudo
            ...
            if not folder:
                # Chercher le dossier parent "Finance" (prioritaire)
                parent_folder = Folder.search([('name', '=', 'Finance')], limit=1)
                
                # Si pas trouvé, chercher "Comptabilité" ou similaire
                ...
                # Si toujours pas de parent, prendre le premier dossier disponible
                if not parent_folder:
                ...
                folder_vals = {
                    'name': 'Scans TVA',
                    'description': 'Documents scannés par le module Scan TVA (tickets, factures)',
                }
                if parent_folder:
                    folder_vals['parent_folder_id'] = parent_folder.id
                    _logger.info("Creation du dossier 'Scans TVA' dans '%s'", parent_folder.name)
                else:
                    _logger.info("Creation du dossier 'Scans TVA' a la racine")
            ...
            _logger.info("Attachment cree: ID=%s, name=%s", attachment.id, attachment.name)
            ...
            # Créer le document dans l'app Documents
            doc_vals = {
                'name': doc_name,
                'folder_id': folder.id,
            ...
            _logger.info("Creation document avec: %s", doc_vals)
            document = Document.sudo().create(doc_vals)
            
            self.documents_document_id = document.id
            _logger.info("Document archive dans Documents: %s (ID: %s, folder: %s)", document.name, document.id, folder.name)
            return True
            
        except Exception as e:
            _logger.error("Erreur lors de la sauvegarde dans Documents: %s", str(e), exc_info=True)
            return False
    ...
    def action_validate(self):
        ...
        # Mettre à jour le document dans l'app Documents avec le partenaire
        if self.documents_document_id and self.partner_id:
            # Vérifier si le modèle existe dans le registre
            try:
                if self.env.registry.get('documents.document'):
                    doc = self.env['documents.document'].sudo().browse(self.documents_document_id)
                    if doc.exists():
                        doc.write({
                ...
                _logger.warning("Impossible de mettre a jour le document archive: %s", str(e))
    ...
    def action_view_document(self):
        ...
        return {
            ...
            'res_id': self.documents_document_id,
            'view_mode': 'form',
            'target': 'current',
        }

    def action_archive_document(self):
        """Archiver manuellement le document dans l'application Documents"""
        self.ensure_one()
            ...
            if result and self.documents_document_id:
                return {
                    ...
                    'params': {
                        ...
                        'message': _("Le document a ete sauvegarde dans l'application Documents (dossier Scans TVA)."),
                        'type': 'success',
                        'sticky': False,
                    }
                }
                ...
                return {
                    ...
                    'params': {
                        'title': _("Erreur"),
                        'message': _("Impossible d'archiver le document. Le module Documents n'est peut-etre pas installe ou accessible."),
                        'type': 'warning',
                        'sticky': True,
                    }
                }
        except Exception as e:
            _logger.error("Erreur archivage manuel: %s", str(e), exc_info=True)
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
