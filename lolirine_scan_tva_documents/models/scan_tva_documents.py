...
class LolirineScanTvaDocuments(models.Model):
    ...
    def _save_to_documents(self):
        ...
        try:
            ...
            # Dans Odoo 18, les dossiers sont des documents.document avec type='folder'
            ...
            if not folder:
                ...
                if not parent_folder:
                    ...
                    parent_folder = Document.search([('type', '=', 'folder')], limit=1)
            ...
            # Créer le document (type='binary' pour un fichier)
            doc_vals = {
                'name': doc_name,
                'type': 'binary',
                'folder_id': folder.id,
            ...
            _logger.info("Document archive: %s (ID: %s, folder: %s)", document.name, document.id, folder.name)
            return True
