...
class KmTrajet(models.Model):
    ...
    def _compute_info_bareme(self):
        ...
        for trajet in self:
            # Chercher le barème actuel pour voiture 7CV (le plus courant)
            bareme = self.env['km.bareme'].search([
                ('puissance_fiscale', '=', '7'),
                ('type_vehicule', '=', 'voiture'),
                ('active', '=', True),
                ('date_debut', '<=', trajet.date or fields.Date.today()),
                '|',
                ('date_fin', '=', False),
                ('date_fin', '>=', trajet.date or fields.Date.today()),
            ...
            if bareme:
                ...
                trajet.info_bareme = "⚠️ Aucun barème configuré pour cette période"
    ...
    @api.onchange('distance_aller', 'aller_retour', 'bareme_id')
    def _onchange_recalc_montant(self):
        """Recalculer le montant quand la distance ou le barème change"""
        ...
        # Calculer le montant
        if self.bareme_id and self.distance > 0:
            self.taux_km = self.bareme_id.taux_jusqu_5000
