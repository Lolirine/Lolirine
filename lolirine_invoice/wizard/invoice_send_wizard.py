...
class LolirineInvoiceSendWizard(models.TransientModel):
    ...
    def _get_default_subject(self, invoice):
        ...
        return f"Envoi de votre facture {invoice.name} - Garde-meubles Lolirine"

    def _get_default_body(self, invoice):
        """Generer le corps de l'email par defaut"""
        lines_html = ""
        for line in invoice.invoice_line_ids.filtered(lambda l: l.display_type == False):
            lines_html += f"<li>{line.name or line.product_id.name or '-'} - {line.quantity:.2f} x {line.price_unit:.2f} EUR = {line.price_subtotal:.2f} EUR</li>"
        
        return f"""
        <div style="font-family: Arial, sans-serif; font-size: 13px; color: #333;">
            <p>Bonjour {invoice.partner_id.name or 'Cher Client'},</p>
            ...
            <p>Cette facture correspond a la periode de location en cours et reprend le detail des prestations facturees.</p>
            
            <table style="margin: 20px 0; border-collapse: collapse; width: 100%; max-width: 500px;">
                <tr style="background-color: #c0392b; color: white;">
                    <th style="padding: 10px; border: 1px solid #ddd; text-align: left;">Information</th>
                ...
                <tr>
                    ...
                    <td style="padding: 10px; border: 1px solid #ddd;">{invoice.amount_untaxed:.2f} EUR</td>
                </tr>
                <tr style="background-color: #f5f5f5;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>TVA</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{invoice.amount_tax:.2f} EUR</td>
                </tr>
                <tr style="background-color: #c0392b; color: white;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Montant Total TTC</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>{invoice.amount_total:.2f} EUR</strong></td>
                </tr>
            </table>
            
            <p><strong>Lignes de facture :</strong></p>
            <ul>
                {lines_html}
            </ul>
            ...
            <p>Pour toute question, contactez-nous a <a href="mailto:gardemeublelolirine@gmail.com">gardemeublelolirine@gmail.com</a> ou au 0497/44 41 46.</p>
