 ...
 * Vanilla JS - compatible Odoo 17/18/19
 * Swap l'image produit quand l'utilisateur change de variante.
 */
(function () {
    'use strict';
    ...
    function initVariantImageSwitcher() {
        ...
        // Charger les données images variantes
        fetchVariantImages(tmplId, function (data) {
            ...
            productDetail.addEventListener('change', function (e) {
                if (e.target.matches('.js_variant_change, input[type="radio"], select')) {
                    setTimeout(function () {
                        updateImage(productDetail, data, originalSrc);
                    }, 100);
                }
            });
    ...
    function updateImage(container, data, originalSrc) {
        ...
        // Priorité aux attributs visuels
    ...
    function getSelectedPtavIds(container) {
        ...
        var checked = container.querySelectorAll(
            'input.js_variant_change:checked, input[type="radio"]:checked'
        );
    ...
    function addThumbnails(container, data) {
        for (var ptavId in data.attribute_values) {
            ...
            var input = container.querySelector(
                'input[value="' + ptavId + '"], [data-value_id="' + ptavId + '"]'
            );
            if (!input) continue;

            var label = input.closest('label')
                || (input.parentElement ? input.parentElement.querySelector('label') : null);
