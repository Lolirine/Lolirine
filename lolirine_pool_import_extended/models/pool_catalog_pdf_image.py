...
class PoolCatalogPdfImage(models.Model):
    ...
    def enhance_image_sharpness(self, image_data):
        """
        Améliore la netteté de l'image pour corriger le flou léger.
        Utilise un unsharp masking subtil.
        """
        if not image_data:
            return image_data
        ...
        try:
            ...
            # Unsharp masking subtil pour la netteté
            # 1. Créer une version floutée
            blurred = img_pil.filter(ImageFilter.GaussianBlur(radius=1.0))
            
            # 2. Calculer la différence (masque de netteté)
            img_array = np.array(img_pil.convert('RGB'))
            blur_array = np.array(blurred.convert('RGB'))
            mask = img_array.astype(float) - blur_array.astype(float)
            
            # 3. Ajouter le masque à l'image originale (facteur 0.3 = subtil)
            sharpened = img_array.astype(float) + mask * 0.3
            sharpened = np.clip(sharpened, 0, 255).astype(np.uint8)
            
            # 4. Légère amélioration du contraste
            enhanced_img = Image.fromarray(sharpened)
            enhancer = ImageEnhance.Contrast(enhanced_img)
            final_img = enhancer.enhance(1.1)  # +10% contraste
