...
class PoolWebsiteCategory(models.Model):
    ...
    def _compute_product_count(self):
        for cat in self:
            # Compter les produits (is_published vient de website_sale)
            cat.product_count = len(cat.product_ids.filtered(
                lambda p: p.is_published if hasattr(p, 'is_published') else True
            ))
