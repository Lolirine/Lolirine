...
Si une information n'est pas visible ou lisible, mets null pour cette valeur."""
        ...
        try:
            ...
            if content and len(content) > 0:
                ...
                # Numéro national - garder uniquement les chiffres et points/tirets
                if data.get('numero_national'):
                    # Vérifier le type du champ
                    field = self._fields.get('x_studio_numro_national')
                    if field:
                        if field.type == 'char':
                            update_vals['x_studio_numro_national'] = data['numero_national']
                        elif field.type == 'integer':
                            # Extraire uniquement les chiffres
                            num_only = ''.join(c for c in data['numero_national'] if c.isdigit())
                            if num_only:
                                update_vals['x_studio_numro_national'] = int(num_only)
                
                # Numéro de carte d'identité
                if data.get('numero_carte'):
                    field = self._fields.get('x_studio_numro_carte_didentit')
                    if field:
                        if field.type == 'char':
                            update_vals['x_studio_numro_carte_didentit'] = data['numero_carte']
                        elif field.type == 'integer':
                            # Extraire uniquement les chiffres
                            num_only = ''.join(c for c in data['numero_carte'] if c.isdigit())
                            if num_only:
                                update_vals['x_studio_numro_carte_didentit'] = int(num_only)
                ...
                if data.get('adresse_ville'):
                    ...
                    # Aussi mettre à jour le champ localité personnalisé si c'est un char
                    field = self._fields.get('x_studio_localit')
                    if field and field.type == 'char':
                        update_vals['x_studio_localit'] = data['adresse_ville']
