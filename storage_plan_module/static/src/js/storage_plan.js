...
(function() {
    ...
    function initializeStoragePlan() {
        ...
        // Bouton prendre rendez-vous (désactivé en mode édition)
        ...
        // Bouton réserver maintenant (désactivé en mode édition)
        ...
        $(document).on('click', '#general-inquiry-btn', function(e) {
            ...
            // Si un lien a été configuré dans l'éditeur
            if (href && href !== '#' && href !== '') {
    ...
    function loadBoxDetails(boxId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var modal = new bootstrap.Modal(document.getElementById('boxDetailsModal'));
                modal.show();
            },
    ...
    function showReservationForm(type) {
        ...
        // Bootstrap 5 modal API
        var detailsModal = bootstrap.Modal.getInstance(document.getElementById('boxDetailsModal'));
        if (detailsModal) {
            detailsModal.hide();
        }
        
        setTimeout(function() {
            var reservationModal = new bootstrap.Modal(document.getElementById('reservationFormModal'));
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (result.success) {
                    ...
                    // Bootstrap 5 modal API
                    var modal = bootstrap.Modal.getInstance(document.getElementById('reservationFormModal'));
                    if (modal) {
                        modal.hide();
                    }
                    setTimeout(function() {
