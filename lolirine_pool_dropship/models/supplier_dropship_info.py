...
class SupplierDropshipInfo(models.Model):
    ...
    price = fields.Monetary(string='Prix catalogue fournisseur HT', required=True, currency_field='currency_id',
                           help="Prix catalogue du fournisseur (= prix de vente client HTVA)")
    
    # Réduction négociée
    discount_percent = fields.Float(string='Réduction négociée (%)', default=0.0,
                                    help="Pourcentage de réduction obtenu auprès du fournisseur (ex: 35%, 40%, 52.5%)")
    negotiated_price = fields.Monetary(string='Prix d\'achat négocié HT', compute='_compute_negotiated_price',
    ...
    def _compute_negotiated_price(self):
        """Calcule le prix d'achat négocié = prix catalogue × (1 - réduction%)"""
        for record in self:
