...
class KmTrajet(models.Model):
    ...
    distance_aller = fields.Float(
        ...
        compute='_compute_distance_aller',
        store=True,
        readonly=False,
        digits=(10, 1),
    )
    ...
    @api.depends('lieu_depart_predef_id', 'destination_predef_id', 'use_predefined')
    def _compute_distance_aller(self):
        for trajet in self:
            if trajet.use_predefined and trajet.lieu_depart_predef_id and trajet.destination_predef_id:
                # Chercher la distance prédéfinie
                distance = trajet.destination_predef_id.get_distance_from(trajet.lieu_depart_predef_id.id)
                if distance > 0:
                    trajet.distance_aller = distance
                    trajet.distance_calculee = True
        ...
        if distance > 0:
            ...
            return {
                ...
                'params': {
                    ...
                    'message': f'Distance calculée: {distance} km',
                    'type': 'success',
                    'sticky': False,
                }
            }
