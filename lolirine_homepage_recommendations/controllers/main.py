    ...
    def _format_products(self, products):
        ...
        for product in products:
            try:
                ...
                # ✅ CORRECTIF: utiliser website_url (slug natif Odoo) avec fallback
                # L'ancienne méthode f'/shop/product/{product.product_tmpl_id.id}' produisait
                # '/shop/product/False' si tmpl_id était vide → '/shop/undefined' côté JS
                tmpl = product.product_tmpl_id
                if tmpl and tmpl.id:
                    product_url = tmpl.website_url if tmpl.website_url else f'/shop/product-{tmpl.id}'
                else:
                    _logger.warning(f"Produit {product.id} sans product_tmpl_id, ignoré")
                    continue
