...
class LolirinePopupConfig(models.Model):
    ...
    @api.model
    def get_all_active_popups(self):
        """Retourne tous les popups actifs"""
        popups = self.search([('active', '=', True)], order='sequence, id')
        result = []
        for popup in popups:
            result.append(self._popup_to_dict(popup))
        return result
    
    def _popup_to_dict(self, popup):
        """Convertit un popup en dictionnaire"""
        ...
        if popup.background_image:
            background_image_url = f'/web/image/lolirine.popup.config/{popup.id}/background_image'
        ...
        return {
            ...
            'show_delay': popup.show_delay * 1000,
    ...
    def get_active_popup(self):
        """Retourne le popup actif le plus prioritaire (compatibilité)"""
        popup = self.search([('active', '=', True)], order='sequence, id', limit=1)
        if popup:
            return self._popup_to_dict(popup)
        return None

    @api.model
    def get_available_boxes(self, limit=5):
        """Retourne la liste des boxes disponibles avec prix TTC"""
        ...
        for box in boxes:
            ...
            price_ht = box.list_price
            taxes = box.taxes_id
            if taxes:
                # Calculer le prix avec taxes
                tax_compute = taxes.compute_all(
                    price_ht, 
                    currency=box.currency_id, 
                    quantity=1, 
                    product=box.product_variant_id,
                    partner=self.env.user.partner_id
                )
                price_ttc = tax_compute['total_included']
            else:
                price_ttc = price_ht
