...
import { onMounted, onWillUnmount, useRef, useState, useEffect } from "@odoo/owl";
...
export class AddressAutocompleteField extends CharField {
    ...
    setup() {
        ...
        onMounted(() => {
            // Délai pour les fenêtres modales
    ...
    async initGooglePlaces() {
        try {
            ...
            // Attendre que l'input soit disponible (important pour les modales)
        ...
        return new Promise((resolve, reject) => {
            const checkInput = () => {
                ...
                if (this.inputRef.el) {
                    ...
                    reject(new Error("Input non trouvé après plusieurs tentatives"));
                }
            };
            checkInput();
        });
    ...
    setupAutocomplete() {
        ...
        try {
            ...
            // Empêcher le formulaire de se soumettre lors de la sélection
            ...
            console.log("KM Expense: Google Places autocomplete initialisé avec succès pour", this.props.name);
        } catch (e) {
