...
class ScraperBase:
    ...
    request_timeout = 6   # secondes (était 15)
    throttle_seconds = 0.3  # délai entre requêtes (était 1.0)
    ...
    def _normalize_query(self, product):
        ...
        Priorité :
        1. x_pool_supplier_ref (vraie ref Fluidra/SCP du catalogue d'origine)
        2. default_code (ref interne Lolirine, type POOL-NA*)
        Puis le nom du produit en complément.
        """
        parts = []

        # Priorité 1 : référence supplier d'origine (clé pour le scraping web)
        supplier_ref = getattr(product, 'x_pool_supplier_ref', None) or ''
        if supplier_ref:
            parts.append(supplier_ref.strip())
        elif product.default_code:
            parts.append(product.default_code.strip())

        # Nom du produit nettoyé
        name = product.name or ''
        # Le ° pose des problèmes d'encodage sur certains sites
    ...
    def _get_supplier_hint(self, product):
        """Retourne le nom du fournisseur pour routing/scoring."""
        sup = getattr(product, 'x_pool_supplier_id', None)
        if sup and sup.name:
            return sup.name.lower()
        return ''
