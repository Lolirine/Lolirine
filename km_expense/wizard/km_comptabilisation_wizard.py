...
class KmComptabilisationWizard(models.TransientModel):
    ...
    def _onchange_periode(self):
        ...
        if not self.annee or not self.annee.isdigit():
            return
        ...
        if self.type_periode == 'mois' and self.mois:
            mois = int(self.mois)
            self.date_debut = date(annee_int, mois, 1)
            if mois == 12:
                self.date_fin = date(annee_int, 12, 31)
            else:
                self.date_fin = date(annee_int, mois + 1, 1) - relativedelta(days=1)
        
        elif self.type_periode == 'trimestre' and self.trimestre:
            trim = int(self.trimestre)
            mois_debut = (trim - 1) * 3 + 1
            self.date_debut = date(annee_int, mois_debut, 1)
            self.date_fin = date(annee_int, mois_debut + 2, 1) + relativedelta(months=1) - relativedelta(days=1)
        
        elif self.type_periode == 'annee':
            self.date_debut = date(annee_int, 1, 1)
            self.date_fin = date(annee_int, 12, 31)
