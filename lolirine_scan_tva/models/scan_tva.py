...
class LolirineScanTva(models.Model):
    ...
    def _save_to_documents(self):
        ...
        try:
            ...
            # Chercher ou créer le dossier "Scans TVA"
            folder = Folder.search([('name', '=', 'Scans TVA')], limit=1)
            if not folder:
                # Chercher le dossier parent "Comptabilité" ou créer à la racine
                parent_folder = Folder.search([('name', 'ilike', 'Comptabilit')], limit=1)
                folder = Folder.create({
                    'name': 'Scans TVA',
                    'parent_folder_id': parent_folder.id if parent_folder else False,
                    'description': 'Documents scannés par le module Scan TVA',
                })
                _logger.info("Dossier 'Scans TVA' cree dans Documents")
            ...
            _logger.warning("Erreur lors de la sauvegarde dans Documents: %s", str(e))
            return False
        ...
        if self.documents_document_id:
            return {
                ...
                'params': {
                    ...
                    'message': _("Le document a ete sauvegarde dans l'application Documents."),
                    'type': 'success',
                    'sticky': False,
                }
            }
            ...
            return {
                ...
                'params': {
                    'title': _("Erreur"),
                    'message': _("Impossible d'archiver le document. Verifiez que le module Documents est installe."),
                    'type': 'warning',
                    'sticky': False,
                }
            }
