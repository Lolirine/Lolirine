...
class PoolCatalogExtractionProduct(models.Model):
    ...
    def action_import_to_odoo(self):
        ...
        try:
            ...
            # Créer les infos dropshipping si coché
            if self.is_dropship_product:
                self._create_dropship_info(product)
    ...
    def _create_dropship_info(self, product):
        ...
        supplier = self.extraction_id.supplier_id
        if not supplier:
            _logger.warning("Pas de fournisseur défini, impossible de créer les infos dropship")
            return False
        ...
        if not partner:
            ...
            _logger.info(f"Partenaire créé pour fournisseur: {partner.name}")
        else:
        ...
        dropship_vals = {
            ...
            'is_active': True,
            'min_order_qty': 1,
            'notes': f"Import automatique depuis extraction OCR le {fields.Datetime.now().strftime('%d/%m/%Y')}",
        }
        ...
        try:
            if existing:
                ...
                result = existing
                _logger.info(f"Info dropship mise à jour pour produit {product.id}")
            else:
                # Créer nouveau
                result = DropshipInfo.create(dropship_vals)
                _logger.info(f"Info dropship créée pour produit {product.id}")
            
            # Marquer le produit comme produit dropshipping
            if 'is_dropship_product' in product._fields and not product.is_dropship_product:
