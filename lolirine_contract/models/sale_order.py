...
class SaleOrder(models.Model):
    ...
    contract_deposit_state = fields.Selection([
        ('not_received', 'Non versée'),
        ('received', 'Versée'),
        ('refunded', 'Remboursée'),
    ], string="État caution", default='not_received', tracking=True)

    contract_deposit_received_date = fields.Date(string="Date versement caution")
    contract_deposit_refund_date = fields.Date(string="Date remboursement caution")
    contract_deposit_move_id = fields.Many2one(
        'account.bank.statement.line',
        string="Transaction bancaire caution",
        domain=[('journal_id', '=', 21)],
    )
...
<div style="font-family: Arial, sans-serif; font-size: 14px; color: #333; line-height: 1.6;">
    ...
    <p>Votre première facture sera générée prochainement. Vous pouvez à tout moment consulter vos documents,
    gérer votre abonnement et mettre à jour vos informations depuis votre portail client personnel.</p>
    <p>Votre Code d'accès vous sera fourni sur place, lors de la signature de votre contrat.
    Vous pouvez prendre contact avec nos services soit en ligne soit par téléphone pour convenir d'un rendez-vous.</p>
    <p style="margin: 20px 0;">
        <a href="{portal_url}" style="background-color: #875a7b; color: #ffffff; padding: 10px 20px;
    ...
    <div style="margin-top: 25px; padding-top: 15px; border-top: 1px solid #dee2e6;">
        <p style="margin: 0;">
            ...
            <span style="color: #6c757d;">Email : <a href="mailto:gardemeublelolirine@gmail.com"
            style="color: #007bff;">gardemeublelolirine@gmail.com</a></span>
        </p>
    </div>
</div>
"""
    ...
    def action_mark_deposit_received(self):
        """Marquer la caution comme versée"""
        self.ensure_one()
        self.write({
            'contract_deposit_state': 'received',
            'contract_deposit_received_date': fields.Date.context_today(self),
        })
        self.message_post(
            body=f"✅ Caution de {self.contract_deposit_amount:.2f}€ marquée comme versée le {fields.Date.context_today(self).strftime('%d/%m/%Y')}",
            message_type='notification'
        )

    def action_mark_deposit_refunded(self):
        """Marquer la caution comme remboursée"""
        self.ensure_one()
        self.write({
            'contract_deposit_state': 'refunded',
            'contract_deposit_refund_date': fields.Date.context_today(self),
        })
        self.message_post(
            body=f"✅ Caution de {self.contract_deposit_amount:.2f}€ remboursée le {fields.Date.context_today(self).strftime('%d/%m/%Y')}",
            message_type='notification'
        )
