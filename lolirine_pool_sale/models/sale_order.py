...
class SaleOrderPool(models.Model):
    ...
    def _get_pool_template_id(self):
        """Trouve l'id du modèle de devis 'Devis Piscine'."""
        tmpl = self.env['sale.order.template'].sudo().search(
            [('name', 'ilike', 'piscine')], limit=1
        )
        if not tmpl:
            tmpl = self.env['sale.order.template'].sudo().search(
                [('name', 'ilike', 'pool')], limit=1
            )
    ...
    def create(self, vals_list):
        """
        Force la séquence PSC AVANT super() et assigne le modèle
        'Devis Piscine' pour éviter d'hériter du modèle garde-meuble.
        """
        ...
        for vals in vals_list:
            if vals.get('is_pool_quote'):
                ...
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'lolirine.pool.sale.order'
                ) or '/'
                # Modèle de devis Piscine (cherché une fois)
    ...
    def action_confirm(self):
        """Re-vérifier la séquence PSC à la confirmation."""
        for rec in self:
    ...
    def write(self, vals):
        """Si is_pool_quote activé : assigner PSC + modèle Devis Piscine."""
        if vals.get('is_pool_quote'):
            for rec in self:
                if not rec.name or rec.name == '/':
                    ...
                    break
            # Assigner le modèle Devis Piscine si pas déjà défini
