...
class SaleOrder(models.Model):
    ...
    dropship_ref = fields.Char(string='Réf. Dropship', copy=False, readonly=True,
                                help="Numéro de référence unique pour le suivi client piscine (POOL-YYYY-XXXXX)")
    ...
    def _compute_is_dropship(self):
        for order in self:
            ...
            order.is_dropship_order = is_dropship

    def write(self, vals):
        """Override: attribuer la réf POOL dès qu'une commande devient dropship"""
        result = super().write(vals)
        # Attribuer la réf dropship sans re-déclencher write (appel direct au parent)
        for order in self:
            if order.is_dropship_order and not order.dropship_ref:
                ref = self.env['ir.sequence'].next_by_code('pool.dropship.order') or '/'
                super(SaleOrder, order).write({'dropship_ref': ref})
        return result
    ...
    def action_confirm(self):
        ...
        # Communication automatique : confirmation client
        for order in self:
            if order.is_dropship_order and order.dropship_ref:
                order._dropship_post_status_message('confirmed')
                order._dropship_send_client_email('confirmed')
    ...
    def action_preview_dropship_order_pdf(self):
        """Prévisualise le PDF de la commande confirmée"""
        self.ensure_one()
        return self.env.ref('sale.action_report_saleorder').report_action(self)
    ...
    def _dropship_post_status_message(self, status_key):
        ...
        messages = {
            'quote_sent': (
                ...
                f"Réf. {ref} — Devis transmis à {self.partner_id.name} ({self.partner_id.email or 'pas d\'email'})<br/>"
