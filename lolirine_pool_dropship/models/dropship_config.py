...
class DropshipConfig(models.Model):
    ...
    # === BRANDING PISCINE ===
    pool_logo = fields.Binary(string='Logo Pool Store',
                               help="Logo pour les documents PDF piscine (devis, factures...)")
    pool_store_name = fields.Char(string='Nom commercial', default='Lolirine Pool Store')
    pool_store_email = fields.Char(string='Email Pool Store', default='info@lolirinepoolstore.be')
    pool_store_phone = fields.Char(string='Téléphone', default='+32 (0)497 44 41 46')
    pool_store_website = fields.Char(string='Site web', default='www.lolirinepoolstore.be')
    pool_store_tagline = fields.Char(string='Slogan',
                                      default='Votre spécialiste piscine & wellness en Belgique')
