...
class StorageIndexation(models.Model):
    ...
    def action_compute_indexation(self):
        ...
        # Récupérer les abonnements actifs (Odoo 18+)
        Subscription = self.env['sale.order'].sudo()
        
        # En Odoo 18, les abonnements actifs ont subscription_state = '3_progress' ou 'in_progress'
        # On cherche les deux pour compatibilité
        ...
        # Si aucun résultat, essayer avec d'autres états possibles
        ...
        for sub in subscriptions:
            ...
            for line in sub.order_line:
                # En Odoo 18, vérifier si la ligne est récurrente
                is_recurring = False
                ...
                elif hasattr(line, 'is_subscription'):
                    is_recurring = line.is_subscription
                else:
                    # Par défaut, considérer toutes les lignes d'un abonnement comme récurrentes
