...
class PurchaseOrder(models.Model):
    ...
    supplier_payment_status = fields.Selection([
        ('not_billed', 'Pas de facture'),
        ('to_pay', 'À payer'),
        ('paid', 'Payée'),
    ], string='Paiement fournisseur', compute='_compute_supplier_payment_status',
       store=True, tracking=True)
    supplier_invoice_ids = fields.Many2many(
        'account.move', string='Factures fournisseur',
        compute='_compute_supplier_invoices')
    supplier_invoice_count = fields.Integer(
        string='Nb factures', compute='_compute_supplier_invoices')
    ...
    def _compute_supplier_payment_status(self):
        for order in self:
            if not order.is_dropship_order:
                order.supplier_payment_status = False
                continue
            invoices = order.invoice_ids.filtered(
                lambda inv: inv.move_type == 'in_invoice'
            )
            if not invoices:
                order.supplier_payment_status = 'not_billed'
            elif all(inv.payment_state in ('paid', 'in_payment', 'reversed') for inv in invoices):
                order.supplier_payment_status = 'paid'
            else:
                order.supplier_payment_status = 'to_pay'
    ...
    def _compute_supplier_invoices(self):
        for order in self:
            ...
            order.supplier_invoice_ids = invoices
            order.supplier_invoice_count = len(invoices)
    ...
    def action_send_dropship_email(self):
        ...
        if self.dropship_sale_id:
            ...
            self.dropship_sale_id._dropship_post_status_message('po_sent')
            self.dropship_sale_id._dropship_send_client_email('po_sent')
    ...
    def action_view_supplier_invoices(self):
        """Voir les factures fournisseur liées"""
        self.ensure_one()
        invoices = self.invoice_ids.filtered(lambda inv: inv.move_type == 'in_invoice')
        if len(invoices) == 1:
            return {
