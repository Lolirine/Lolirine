...
class AccountMove(models.Model):
    ...
    def _create_auto_km_trajet(self):
        ...
        if self.km_trajet_auto_created:
            return False
        
        if not self.partner_id:
            return False
        
        if not self.partner_id.km_trajet_auto:
            return False
        
        # Vérifier le type de facture
        if self.move_type in ('in_invoice', 'in_refund'):
            if self.partner_id.km_trajet_type not in ('fournisseur', 'les_deux'):
                return False
            ...
            if self.partner_id.km_trajet_type not in ('client', 'les_deux'):
                return False
            trajet_type = 'client'
