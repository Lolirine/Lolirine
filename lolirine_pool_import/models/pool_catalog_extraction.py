...
class PoolCatalogExtraction(models.Model):
    ...
    furniture_variants_data = fields.Text(
        ...
        help='Ex: [{"name": "BUTTERFLY", "code": "32474AW360", "purchase_price": 20434}]'
    )
    shell_colors_data = fields.Text(
        string='Couleurs cuve (JSON)',
        help='Ex: [{"name": "Effet Inox", "code_number": "27"}]'
    )
    product_options_data = fields.Text(
        string='Options produit (JSON)',
        help='Ex: [{"name": "Nordic Insulation", "code_suffix": "359", "price_extra": 312}]'
    )
...
SPAS / JACUZZIS / WELLNESS - EXTRACTION SPÉCIALE:
Pour les pages de spas, jacuzzis ou produits wellness, extrais ces informations SUPPLÉMENTAIRES dans le JSON:

"furniture_variants": [
    {"name": "BUTTERFLY", "code": "32474AW360", "full_code": "MA20M SC", "purchase_price": 20434.00},
    {"name": "GRAFITO", "code": "32474AK360", "full_code": "MA20M SC", "purchase_price": 20434.00},
    {"name": "THUNDER", "code": "32474AR360", "full_code": "MA20M SC", "purchase_price": 20434.00}
],
"shell_colors": [
    {"name": "Effet Inox", "code_number": "27"},
    {"name": "Blanc", "code_number": "44"},
    {"name": "Sterling Marbré", "code_number": "59"},
    {"name": "Cameo", "code_number": "72"}
],
"product_options": [
    {"name": "Sans option", "code_suffix": "360", "price_extra": 0},
    {"name": "Nordic Insulation System", "code_suffix": "359", "price_extra": 312.00},
    {"name": "Nordic Insulation System + Audio 4.1", "code_suffix": "355", "price_extra": 1457.00}
]

RÈGLES POUR LES SPAS:
- Si le tableau contient des lignes "Avec meuble BUTTERFLY/GRAFITO/THUNDER/CREEK/INFINITY/etc.", ce sont des furniture_variants
- Si une section "Couleur de la cuve" est visible, extrais toutes les couleurs dans shell_colors
- Si une section "Options" est visible avec des suppléments de prix, extrais-les dans product_options
- Le code_suffix des options remplace les 3 derniers chiffres du code produit
- Le code_number des couleurs s'ajoute à la fin du code avec le préfixe "CL"
- Le extraction_type DOIT être "variants" quand des meubles/couleurs/options sont détectés
- Les prix dans furniture_variants sont les prix unitaires HT de chaque combinaison meuble
- IMPORTANT: Pour les spas, il faut créer UNE SEULE entrée dans "products" avec les specs techniques,
  les variantes meuble/couleur/option sont dans les champs dédiés ci-dessus
...
Réponds UNIQUEMENT avec le JSON, sans texte supplémentaire ni backticks."""
    ...
    def _process_extraction_result(self, data):
        ...
        # VARIANTES MULTI-DIMENSIONNELLES (SPAS, etc.)
        # =============================================
        furniture_variants = data.get('furniture_variants', [])
        shell_colors = data.get('shell_colors', [])
        product_options = data.get('product_options', [])
        
        if furniture_variants:
            self.furniture_variants_data = json.dumps(furniture_variants, ensure_ascii=False)
            _logger.info(f"🛋️ {len(furniture_variants)} variante(s) de meuble détectée(s)")
        
        if shell_colors:
            self.shell_colors_data = json.dumps(shell_colors, ensure_ascii=False)
            _logger.info(f"🎨 {len(shell_colors)} couleur(s) de cuve détectée(s)")
        
        if product_options:
            self.product_options_data = json.dumps(product_options, ensure_ascii=False)
            _logger.info(f"⚙️ {len(product_options)} option(s) détectée(s)")
        
        # Si des variantes spa sont détectées, forcer le type "variants"
        if furniture_variants or shell_colors:
            self.extraction_type = 'variants'
            _logger.info("Type d'extraction forcé à 'variants' (variantes spa détectées)")
...
Réponds UNIQUEMENT avec le JSON."""
    ...
    def action_import_all(self):
        ...
        # Soit multi-produits classiques, soit spa avec multi-dimensions (même avec 1 produit extrait)
        has_spa_variants = bool(self.furniture_variants_data or self.shell_colors_data or self.product_options_data)
        
        if self.extraction_type == 'variants' and (len(products_to_import) > 1 or has_spa_variants):
    ...
    def _import_as_single_product_with_variants(self, products_to_import):
        ...
        Gère les variantes mono-attribut (classique) ET multi-attribut (spas: meuble + couleur + option).
        """
        self.ensure_one()
        _logger.info(f"=== Import avec variantes: {len(products_to_import)} produit(s) extrait(s) ===")
        
        # Vérifier si on a des variantes spa multi-dimensionnelles
        has_spa_variants = bool(self.furniture_variants_data or self.shell_colors_data or self.product_options_data)
        
        if has_spa_variants:
            return self._import_spa_multi_variants(products_to_import)
        else:
            return self._import_classic_variants(products_to_import)
    
    def _import_spa_multi_variants(self, products_to_import):
        """
        Import spécifique pour les spas avec variantes multi-dimensionnelles:
        - Meuble (BUTTERFLY, GRAFITO, THUNDER, etc.)
        - Couleur cuve (Effet Inox, Blanc, Sterling Marbré, Cameo)
        - Options (Nordic Insulation, Audio, etc.)
        """
        self.ensure_one()
        _logger.info("=== Import SPA multi-variantes ===")
        ...
        if self.furniture_variants_data:
            try:
                furniture_variants = json.loads(self.furniture_variants_data)
            except (json.JSONDecodeError, TypeError):
                _logger.warning("Impossible de parser furniture_variants_data")
        
        if self.shell_colors_data:
            try:
                shell_colors = json.loads(self.shell_colors_data)
            except (json.JSONDecodeError, TypeError):
                _logger.warning("Impossible de parser shell_colors_data")
        
        if self.product_options_data:
            try:
                product_options = json.loads(self.product_options_data)
            except (json.JSONDecodeError, TypeError):
                _logger.warning("Impossible de parser product_options_data")
        
        _logger.info(f"Meubles: {len(furniture_variants)}, Couleurs: {len(shell_colors)}, Options: {len(product_options)}")
        
        # =============================================
        # CRÉER LES ATTRIBUTS ET VALEURS
        # =============================================
        attribute_line_ids = []
        
        # --- ATTRIBUT MEUBLE ---
        furniture_value_data = {}  # {value_name: {code, price, attr_value_id}}
        if furniture_variants:
            attr_meuble = ProductAttribute.search([('name', '=', 'Meuble')], limit=1)
            if not attr_meuble:
                attr_meuble = ProductAttribute.create({
                    'name': 'Meuble',
                    'display_type': 'radio',
                    'create_variant': 'always',
                })
                _logger.info(f"Attribut 'Meuble' créé (ID: {attr_meuble.id})")
            
            meuble_value_ids = []
            for fv in furniture_variants:
                fv_name = fv.get('name', '').strip()
                if not fv_name:
                    continue
                
                attr_val = ProductAttributeValue.search([
                    ('attribute_id', '=', attr_meuble.id),
                    ('name', '=', fv_name)
                ], limit=1)
                ...
                meuble_value_ids.append(attr_val.id)
                furniture_value_data[fv_name] = {
                    'code': fv.get('code', ''),
                    'full_code': fv.get('full_code', ''),
                    'purchase_price': self._parse_price(fv.get('purchase_price', 0)),
                    'attr_value_id': attr_val.id,
                }
            
            if meuble_value_ids:
                attribute_line_ids.append((0, 0, {
                    'attribute_id': attr_meuble.id,
                    'value_ids': [(6, 0, meuble_value_ids)],
                }))
                _logger.info(f"Attribut Meuble avec {len(meuble_value_ids)} valeurs")
        
        # --- ATTRIBUT COULEUR CUVE ---
        if shell_colors:
            attr_couleur = ProductAttribute.search([('name', '=', 'Couleur Cuve')], limit=1)
            if not attr_couleur:
                attr_couleur = ProductAttribute.create({
                    'name': 'Couleur Cuve',
                    'display_type': 'color',
                    'create_variant': 'always',
                })
                _logger.info(f"Attribut 'Couleur Cuve' créé (ID: {attr_couleur.id})")
            
            couleur_value_ids = []
            for sc in shell_colors:
                sc_name = sc.get('name', '').strip()
                if not sc_name:
                    continue
                
                attr_val = ProductAttributeValue.search([
                    ('attribute_id', '=', attr_couleur.id),
                    ('name', '=', sc_name)
                ], limit=1)
                ...
                couleur_value_ids.append(attr_val.id)
            
            if couleur_value_ids:
                attribute_line_ids.append((0, 0, {
                    'attribute_id': attr_couleur.id,
                    'value_ids': [(6, 0, couleur_value_ids)],
                }))
                _logger.info(f"Attribut Couleur Cuve avec {len(couleur_value_ids)} valeurs")
        
        # --- ATTRIBUT OPTIONS ---
        if product_options:
            # Filtrer les options avec un prix extra > 0 (ignorer "Sans option")
            real_options = [opt for opt in product_options if self._parse_price(opt.get('price_extra', 0)) > 0]
            
            if real_options:
                attr_option = ProductAttribute.search([('name', '=', 'Option')], limit=1)
                if not attr_option:
                    attr_option = ProductAttribute.create({
                        'name': 'Option',
                        'display_type': 'radio',
                        'create_variant': 'no_variant',
                    })
                    _logger.info(f"Attribut 'Option' créé (ID: {attr_option.id})")
                
                option_value_ids = []
                option_price_data = {}  # {value_name: price_extra}
                ...
                for opt in real_options:
                    ...
                    option_value_ids.append(attr_val.id)
                    option_price_data[opt_name] = self._parse_price(opt.get('price_extra', 0))
                
                if option_value_ids:
                    attribute_line_ids.append((0, 0, {
                        'attribute_id': attr_option.id,
                        'value_ids': [(6, 0, option_value_ids)],
                    }))
                    _logger.info(f"Attribut Option avec {len(option_value_ids)} valeurs")
        
        if not attribute_line_ids:
            _logger.warning("Aucun attribut créé, fallback vers import classique")
            return self._import_classic_variants(products_to_import)
        
        # =============================================
        # PRÉPARER LE TEMPLATE PRODUIT
        # =============================================
        ...
        if not base_name:
            ...
            # Nettoyer les suffixes de variante
            for suffix in [base_product.capacity, base_product.variant_name, base_product.type_code]:
                if suffix and suffix in base_name:
                    for pattern in [f" - {suffix}", f" ({suffix})", f"- {suffix}", f"({suffix})", f" {suffix}"]:
                        if pattern in base_name:
                            base_name = base_name.replace(pattern, "")
            base_name = ' '.join(base_name.split()).strip()
        
        _logger.info(f"Nom du template: {base_name}")
        
        # Déterminer le prix de base (premier meuble ou premier produit)
        base_price = 0
        if furniture_value_data:
            first_furniture = list(furniture_value_data.values())[0]
            base_price = first_furniture.get('purchase_price', 0)
        elif base_product.purchase_price:
            base_price = base_product.purchase_price
        
        ref_code = base_product.reference or base_product.type_code or str(self.id)
        
        vals = {
            'name': base_name or 'Spa avec variantes',
        ...
        custom_fields = [
            ...
            ('x_pool_import_source', f'Extraction OCR {self.id} (spa multi-variantes)'),
        ]
        ...
        try:
            # Créer le template
            _logger.info(f"Création du template spa multi-variantes...")
            template = ProductTemplate.create(vals)
            _logger.info(f"Template créé: {template.name} (ID: {template.id})")
            
            # Compter les variantes générées
            variant_count = len(template.product_variant_ids)
            _logger.info(f"Nombre de variantes générées par Odoo: {variant_count}")
            
            # =============================================
            # METTRE À JOUR LES PRIX DES VARIANTES
            # =============================================
            # Pour les variantes Meuble: appliquer le prix spécifique de chaque meuble
            if furniture_value_data:
                for variant in template.product_variant_ids:
                    ...
                    for ptav in ptavs:
                        ...
                        if attr_name == 'Meuble' and value_name in furniture_value_data:
                            fv_data = furniture_value_data[value_name]
                            variant_price = fv_data.get('purchase_price', base_price)
                            if fv_data.get('code'):
                                variant_code_parts.append(fv_data['code'])
                    
                    # Mettre à jour la variante
                    write_vals = {
                        'standard_price': variant_price,
                    }
                    if variant_code_parts:
                        write_vals['default_code'] = f"POOL-{'-'.join(variant_code_parts)}"
                    
                    variant.write(write_vals)
                    _logger.info(f"Variante mise à jour: {variant.display_name}, prix: {variant_price}")
            
            # Appliquer les suppléments de prix pour les options (via price_extra sur PTAV)
            if product_options and 'option_price_data' in dir():
                pass  # Les options no_variant n'ont pas de price_extra standard
            
            # Appliquer les price_extra sur les PTAV des options
            if product_options:
                for ptav_line in template.attribute_line_ids:
                    if ptav_line.attribute_id.name == 'Option':
                        for ptav in ptav_line.product_template_value_ids:
                            opt_name = ptav.product_attribute_value_id.name
                            # Chercher le price_extra correspondant
                            for opt in product_options:
                                if opt.get('name', '').strip() == opt_name:
                                    price_extra = self._parse_price(opt.get('price_extra', 0))
                                    if price_extra > 0:
                                        ptav.write({'price_extra': price_extra})
                                        _logger.info(f"Option '{opt_name}': price_extra = {price_extra}€")
            
            # Ajouter les attributs informatifs
            ...
            # Marquer les produits extraits comme importés
            ...
            _logger.info(f"✅ Spa importé: {template.name} avec {variant_count} variantes")
            ...
            _logger.error(f"Erreur création spa multi-variantes: {str(e)}")
            ...
            raise UserError(_("Erreur lors de la création du spa avec variantes: %s") % str(e))
    
    def _import_classic_variants(self, products_to_import):
        """
        Import classique pour les variantes mono-attribut (PAC, raccords PVC, etc.).
        Un seul attribut (Capacité, Diamètre, Puissance) avec plusieurs valeurs.
        """
        self.ensure_one()
        _logger.info(f"=== Import classique avec variantes: {len(products_to_import)} variantes ===")
        
        ProductTemplate = self.env['product.template']
        ProductAttribute = self.env['product.attribute']
        ProductAttributeValue = self.env['product.attribute.value']
        
        # Déterminer l'attribut principal
        ...
        # Préparer les valeurs d'attribut
        attribute_values = []
        variant_data = {}
        ...
        # E-commerce
        if 'website_description' in ProductTemplate._fields and base_product.description_fr:
            vals['website_description'] = base_product._format_website_description()
        if 'is_pool_product' in ProductTemplate._fields:
            vals['is_pool_product'] = True
        if 'is_published' in ProductTemplate._fields:
            vals['is_published'] = True
        ...
        try:
            ...
            _logger.error(f"Erreur création variantes classiques: {str(e)}")
            _logger.exception("Traceback:")
            for prod in products_to_import:
                prod.write({
                    'state': 'error',
    ...
    def action_update_existing_product_images(self):
        """
        Met à jour les images des produits Odoo existants à partir des images
        découpées (ImageCropSelector) ou extraites depuis le catalogue.
        
        Workflow:
        1. Pour chaque produit extrait, trouver le produit Odoo correspondant
        2. Assigner l'image recadrée comme image principale du produit
        """
        self.ensure_one()
        ...
        for ext_product in self.extracted_product_ids:
            # Trouver le produit Odoo lié
            product = ext_product.product_id or ext_product.existing_product_id
            
            if not product:
                # Essayer un matching manuel par référence/code
                product = self._find_matching_product(ext_product)
            
            if not product:
                not_found += 1
                _logger.info(f"Pas de produit Odoo trouvé pour: {ext_product.name}")
                continue
            
            # Trouver la meilleure image
            image = self._get_best_image_for_product(ext_product)
            
            if not image:
                no_image += 1
                _logger.info(f"Pas d'image disponible pour: {ext_product.name}")
                continue
            
            # Mettre à jour l'image du produit
            product.write({'image_1920': image})
            
            # Mettre à jour le lien si pas encore fait
            if not ext_product.product_id:
                ext_product.product_id = product.id
            
            updated += 1
            _logger.info(f"Image mise à jour pour: {product.name} (ID={product.id})")
        
        # Notification résultat
        msg_parts = []
        if updated:
            msg_parts.append(f'{updated} image(s) mise(s) à jour')
        if not_found:
            msg_parts.append(f'{not_found} produit(s) non trouvé(s)')
        if no_image:
            msg_parts.append(f'{no_image} sans image à assigner')
    ...
    def _find_matching_product(self, ext_product):
        """Cherche un produit Odoo correspondant à un produit extrait"""
        ProductTemplate = self.env['product.template']
        
        # Par référence fournisseur
        if ext_product.reference:
            product = ProductTemplate.search([
                ('x_pool_supplier_ref', '=', ext_product.reference)
            ], limit=1)
            if product:
                return product
        
        # Par code type
        if ext_product.type_code:
            product = ProductTemplate.search([
                '|',
                ('x_pool_supplier_ref', '=', ext_product.type_code),
                ('default_code', 'ilike', ext_product.type_code),
            ], limit=1)
            if product:
                return product
    ...
    def _get_best_image_for_product(self, ext_product):
        """Retourne la meilleure image disponible pour un produit extrait.
        Priorité: image recadrée > image matchée catalogue > image extraction"""
        
        # 1. Image directement sur le produit extrait (pré-matchée)
        if ext_product.product_image:
            return ext_product.product_image
        
        # 2. Images du catalogue matchées par référence
        if self.catalog_image_ids:
            for cat_img in self.catalog_image_ids:
                if cat_img.image and cat_img.reference:
                    if (ext_product.reference and cat_img.reference == ext_product.reference) or \
                       (ext_product.type_code and cat_img.reference == ext_product.type_code):
                        return cat_img.image
        
        # 3. Image principale de l'extraction
        if self.extracted_product_image:
            return self.extracted_product_image
    ...
    # =============================================
    # IMAGE CROP SELECTOR - BACKEND INTEGRATION
    # =============================================
    
    @api.model
    def detect_product_images(self, image_base64, media_type='image/png'):
        """
        Détecte les zones contenant des images de produits dans une capture de catalogue.
        Appelé par le composant ImageCropSelector via RPC.
        
        :param image_base64: Image en base64 (sans le préfixe data:...)
        :param media_type: Type MIME de l'image (image/png ou image/jpeg)
        :return: Liste de zones [{label, x_percent, y_percent, width_percent, height_percent, confidence}]
        """
        api_key = self.env['ir.config_parameter'].sudo().get_param('pool.claude_api_key')
        
        if not api_key:
            _logger.warning("Clé API Claude non configurée pour la détection d'images")
            return []
        ...
        detect_prompt = """Analyse cette image qui est une capture d'écran d'une page de catalogue de produits de piscine/spa.

Identifie TOUTES les images de produits visibles dans cette capture. Pour chaque image de produit trouvée, donne ses coordonnées en pourcentage de l'image totale.

IMPORTANT:
- Ne retourne QUE les images de PRODUITS (pompes, filtres, robots, spas, raccords, etc.)
- Ignore les logos, icônes, QR codes, pictogrammes, et éléments décoratifs
- Ignore les tableaux de texte et les zones de texte pur
- Chaque zone doit encadrer l'image du produit au plus près, sans inclure de texte autour
- Les coordonnées sont en % de la largeur/hauteur totale de l'image

Réponds UNIQUEMENT avec un JSON valide (pas de markdown, pas de backticks), sous cette forme exacte :
{
    "images": [
        {
            "label": "Nom descriptif court du produit",
            "x_percent": 5.2,
            "y_percent": 10.8,
            "width_percent": 15.0,
            "height_percent": 20.3,
...
Si aucune image de produit n'est trouvée, retourne : {"images": []}"""
        ...
        try:
            ...
            # Filtrer par confiance >= 0.5
            images = [img for img in images if img.get('confidence', 0.5) >= 0.5]
            
            _logger.info(f"🔍 ImageCropSelector auto-détection: {len(images)} zone(s) trouvée(s)")
            return images
            
        except requests.exceptions.RequestException as e:
            _logger.error(f"Erreur API Claude (détection images): {str(e)}")
            return []
        except (json.JSONDecodeError, KeyError) as e:
            _logger.error(f"Erreur parsing réponse Claude: {str(e)}")
            return []
        except Exception as e:
            _logger.error(f"Erreur inattendue détection images: {str(e)}")
            return []
    
    def save_cropped_images(self, image_data_list):
        """
        Sauvegarde les images découpées par le composant ImageCropSelector.
        Appelé depuis le frontend après confirmation des sélections.
        
        :param image_data_list: Liste de dicts [{sequence, label, base64, 
        ...
        # Supprimer les anciennes images manuellement découpées
        manual_images = self.catalog_image_ids.filtered(
            lambda img: not img.reference and not img.type_code
        )
        if manual_images:
            manual_images.unlink()
        ...
        for img_data in image_data_list:
            ...
            try:
                CatalogImage.create({
                    ...
                    'description': img_data.get('label', f'Image {created_count + 1}'),
                    'image': base64_data,
                    'x_percent': img_data.get('x_percent', 0),
                    'y_percent': img_data.get('y_percent', 0),
                    'width_percent': img_data.get('width_percent', 0),
                    'height_percent': img_data.get('height_percent', 0),
                })
                created_count += 1
            except Exception as e:
                _logger.warning(f"Erreur sauvegarde image découpée: {str(e)}")
                continue
        
        # Mettre à jour le flag de détection d'image
        if created_count > 0:
            self.product_image_detected = True
            # Utiliser la première image comme image principale
            first_img = self.catalog_image_ids.sorted('sequence')[:1]
            if first_img and first_img.image:
                self.extracted_product_image = first_img.image
            
            # Re-matcher les images aux produits extraits
            if self.extracted_product_ids:
                self._match_images_to_products()
        
        _logger.info(
            f"✂️ ImageCropSelector: {created_count} image(s) sauvegardée(s) "
            f"pour extraction {self.id}"
        )
